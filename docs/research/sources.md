# Ourea — working source register

This is the project provenance register, not a formally styled bibliography.

## Innovate4Cities 2026 official

### Hackathon 2026 rules / criteria / submissions / webinars
`https://www.innovate4cities.org/hackathon/hackathon2026/`

Key project constraints used:
- city-scale;
- climate challenge;
- evidence-based and AI-driven;
- bonus for Transportation / Informal or Marginalized Settlements / Misinformation and Disinformation;
- ≤20-slide concept PDF;
- demonstration / proof of concept;
- 6-month pilot plan;
- relevance, innovation, impact, presentation, feasibility, viability;
- Office Hours on 19 Aug 2026.

### Program / previous winners
`https://www.innovate4cities.org/ai-x-city-climate-action/`

### 2025 winners
`https://www.innovate4cities.org/blog/winners-2025-ai-city-climate-action-hackathon-at-climate-week-nyc/`

### 2026 launch / FloodFact AI
`https://www.innovate4cities.org/blog/the-2026-ai-x-city-climate-action-hackathon-is-officially-live/`

## Informal settlements / community evidence (learning sources, not model coefficients)

These sources inform safeguards and the pilot protocol. They are **not** converted into optimizer weights.

### Innovate4Cities 2026 informal or marginalized settlements bonus
`https://www.innovate4cities.org/hackathon/hackathon2026/`

Use:
city-scale, climate, evidence, AI-driven solutions; bonus for integrating underserved communities through inclusive urban planning and service delivery.

### Smith et al. (2020) — negotiated landslide-risk mitigation in informal settlements, Medellín
`https://doi.org/10.5751/ES-11337-250119`

Use:
a technically justified intervention can fail socially if attachment, livelihoods, trust, local risk perception and participation are ignored.

### Vilar and Cartes (2016) — urban design and social capital in Moravia, 2004–2014
`https://doi.org/10.1016/j.sbspro.2015.12.008`

Use:
Moravia as a historically marginalized neighborhood where physical upgrading interacts with social capital; not a proving-ground ranking.

### Archivo Vivo de Moravia
`https://www.ucl.ac.uk/bartlett/research-projects/2024/jan/living-archives-archivo-vivo-weaving-gendered-histories-territorial-reclamation`

`https://archivovivomoravia.org/`

Use:
territorial reclamation, gendered memory and community archives as evidence of attachment, not as numeric scores.

### Sustainable Neighborhood Plan for Moravia
`https://ecocitybuilders.org/wp-content/uploads/2018/10/Moravia_Sustainable_Neighborhood_Plan_UNEP_English_10-01-18.pdf`

Use:
neighborhood-scale participation and livelihoods as design context for safeguards.

See `docs/research/case-studies/moravia.md`.

## Official Medellín spatial data

### 2024 terrain
`https://www.medellin.gov.co/servidormapas/rest/services/ServiciosImagen/Modelo_digital_de_elevacion_medellin_2024/ImageServer`

### Cadastral constructions
`https://www.medellin.gov.co/servidormapas/rest/services/vivienda_ciudad_terri/VA_ConsultaOperadorCatastral_geo/MapServer/14`

### Access / nomenclature network
`https://www.medellin.gov.co/servidormapas/rest/services/transporte/VM_Transporte/MapServer/0`

### DANE census blocks
`https://geoportal.dane.gov.co/mparcgis/rest/services/MGN2018/Serv_CapaManzanaInt_2018/MapServer/0`

## Official city-scale population / socioeconomic inputs

### Population projections 2018–2030
`https://www.medellin.gov.co/es/wp-content/uploads/2022/08/5.-Proyecciones-poblacionales-a-nivel-Barrios-y-Veredas-Medellin-2018-%E2%80%93-2030.xlsx`

Ourea uses 2026 total population for the 249 official urban barrio records.

### IMCV / AMPI-AMPI 2023
`https://www.medellin.gov.co/es/wp-content/uploads/2022/08/socializacionIMCV2023.xlsx`

Ourea uses official comuna-level AMPI-AMPI as the socioeconomic city-screen component.

## Medellín climate / adaptation planning

### Medellín climate-risk assessment
`https://www.medellin.gov.co/es/wp-content/uploads/2024/08/60.8.-Evaluacion-de-Riesgos-Climaticos_CRA.pdf`

### Prioritized climate actions
`https://www.medellin.gov.co/es/wp-content/uploads/2024/08/60.9.-Reporte-de-acciones-priorizadas.pdf`

### Comuna 8 local development plan
`https://www.medellin.gov.co/es/wp-content/uploads/2024/07/PDL_C8.pdf`

### Recent Comuna 8 adaptation / community work
`https://www.hw.ac.uk/news/2026/how-one-medellin-neighbourhood-changed-how-a-whole-city-plans-for-climate-change`

## Adjacent Medellín early-warning / decision-support work

### Inform@Risk / Bello Oriente
`https://nhess.copernicus.org/articles/24/1843/`

Use:
reason not to pitch OUREA as another local landslide early-warning system.

### Sapena et al. 2023 — citywide LEWS prioritization
`https://nhess.copernicus.org/articles/23/3913/2023/`

Use:
closest adjacent decision-support work; prioritizes monitoring instrumentation, whereas OUREA prioritizes physical adaptation portfolios.

## Historical rainfall bridge

### SIATA event report — 18 June 2022
`https://siata.gov.co/reporte_eventos/2597-Informe_Evento_Precipitacion_%282022-06-18%29.pdf`

### SIATA event report — 19 June 2022
`https://siata.gov.co/reporte_eventos/informes_parciales/2598-Informe_Evento_Precipitacion_%282022-06-19%29_parcial.pdf`

These support the antecedent-rainfall design but do not replace the requested raw station series.

## Local RWH evidence

### Santa Elena 2023 — 50 × 1,000 L systems + official budget ceiling
Invitation/specification:
`https://www.fundacionudea.com/sitio/img/nvitaci%C3%B3n_2_a_participar_C90_hog.pdf`

Evaluation confirming official budget ceiling:
`https://www.fundacionudea.com/sitio/img/16.%20evaluaci%C3%B3n%20a%20participar_comuna%2090%20hogares%20y%20sector%20comercio-2023.pdf`

Ourea evidence anchor:
~COP 2.119M / installed 1,000 L household system as an official procurement **ceiling**, not a transferable Comuna 8 market/unit price.

### Comuna 1 2025 — 130 × 250 L technical specification
`https://www.fundacionudea.com/sitio/img/Documentos%20referencia%202/Segunda_Invitaci%C3%B3n_FUDEA_C1_HOGA.pdf`

### San Sebastián de Palmitas 2025 — isolated RWH budget component
`https://www.fundacionudea.com/sitio/img/Documentos%20Referencia/Adendas/Evaluaciones/1.%20evaluacio%CC%81n%20a%20participar_c50-%20hogares.pdf`

### Comuna 8 project portal
`https://ppaunclic.medellin.gov.co/reportes/proyectos/`

Use:
local rainwater-capture implementation relevance.

## Local mitigation / hydraulic cost scale

### Comuna 8 DAGRD mitigation/bioengineering project
`https://www.medellin.gov.co/ndesarrollo/wp-content/uploads/FICHAS/C8/2019/FP/19_FP_PP_C8_DAGRD.pdf`

### Medellín 2026 hydraulic works
`https://www.medellin.gov.co/es/sala-de-prensa/noticias/medellin-inicia-el-2026-con-seis-nuevas-obras-hidraulicas-en-ejecucion-y-el-reto-de-intervenir-91-quebradas/`

`https://www.medellin.gov.co/es/sala-de-prensa/noticias/la-alcaldia-de-medellin-acelera-obras-de-mi-rio-mis-quebradas-para-mitigar-inundaciones-y-recuperar-espacio-publico/`

`https://www.medellin.gov.co/es/sala-de-prensa/noticias/mi-rio-mis-quebradas-avanza-con-el-93-de-ejecucion-de-obras-en-la-quebrada-la-honda-en-el-nororiente-de-la-ciudad/`

Use:
implementation/budget scale and evidence of heterogeneous project scope—not universal unit costs.

## Transferable intervention mechanisms

### Rainwater harvesting / stormwater performance
`https://www.sciencedirect.com/science/article/abs/pii/S0301479722023878`

### Vegetation hydro-mechanical effects
`https://pubmed.ncbi.nlm.nih.gov/38485024/`

### Vegetation reinforcement parameterization
`https://link.springer.com/article/10.1007/s10346-024-02300-1`

Use:
mechanisms and uncertainty ranges to investigate, not direct Medellín calibration.

## Robust decision-making under uncertainty

`https://www.sciencedirect.com/science/article/pii/S2212096325000658`

`https://openknowledge.worldbank.org/entities/publication/cb452737-0fb0-59a1-88d9-162af7882938`

`https://www.rand.org/global-and-emerging-risks/centers/pardee/dmdu-decision-making-under-deep-uncertainty/robust-decision-making.html`

Use:
methodological basis for evaluating plans across futures instead of optimizing against one deterministic forecast.

## Parameter adoption policy

A value does **not** become a OUREA parameter merely because it appears in a source.

Final parameters must be tagged as:
- local measurement;
- local engineering estimate;
- local procurement/project evidence;
- transferable literature prior;
- explicit planning prior.

Current numerical source of truth:
`frontend/src/config/modelParameters.json`
